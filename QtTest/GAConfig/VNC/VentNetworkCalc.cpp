...
static double MeanSquare( DValueArray& dvalues )
{
    // 所有数据的绝对值的均方差
    int n = dvalues.size();
...
static bool Filter_DFS( Digraph& dg, ArcFilter& tf, Digraph::Node u, Digraph::Node v, EdgeArray& p )
{
...
void VentNetworkCalc::findCycleByTree( EdgeArray& tree, CycleList& cl )
{
    ...
    for( int i = 0; i < n; i++ )
    {
        ...
        //  查找v->u的一条路径
        EdgeArray p;
        if( !Filter_DFS( dg, tf, v, u, p ) ) continue;
...
bool VentNetworkCalc::doIterate( int count )
{
    ...
    while( !ret && k <= count )
    {
        ...
        if( IsMeetErrorRequirement( dqArray, precise ) &&
                IsMeetErrorRequirement( dfArray, precise ) )
        {
            ret = true;
            break;
        }
...
static bool DFS_Helper( Digraph& dg, Digraph::Node u, Digraph::Node v, EdgeArray& p )
{
    ...
    for( DPath::ArcIt e( up ); e != INVALID; ++e )
    {
        p.push_back( e );
    }
...
static double CaclPathH( Digraph& dg, VNC_EdgeDataMap& ed, VNC_FanDataMap& fd, EdgeArray& p )
{
    double H = 0;
    ...
    {
        Digraph::Arc e = p[i];
        VNC_EdgeData* pEdgeData = ed[e];
        //VNC_FanData* pFanData = fd[e];

        H += pEdgeData->f0();
        /*if(pFanData != 0)
        {
        	H += pFanData->h0(pEdgeData->q);
        }*/
    }
    return H;
}

double TotalH( Digraph& dg, VNC_EdgeDataMap& ed, VNC_FanDataMap& fd )
{
    Digraph::Node sn, tn;
    AddVirtualSTNode( dg, sn, tn );               // 处理多源多汇网络
    ...
    // dfs搜索一条通路，计算阻力
    EdgeArray p;
    DFS_Helper( dg, sn, tn, p );

    // 计算除通风动力以外的总阻力
    double H = CaclPathH( dg, ed, fd, p );
