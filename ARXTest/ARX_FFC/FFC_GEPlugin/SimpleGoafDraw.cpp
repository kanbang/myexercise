...
static void DrawBoundary( AcGiWorldDraw* mode, const AcGePoint3dArray& polygon, double offset, bool isInner )
{
    AcGePoint3dArray offset_polygon;
    if( OffSetPolygon( polygon, offset, isInner, offset_polygon ) )
    {
        DrawPolygon( mode, offset_polygon, false );
    }
}

static void DrawGoaf( AcGiWorldDraw* mode, const AcGePoint3dArray& polygon, double offset, double scale, bool isInner )
{
    AcGePoint3dArray offset_polygon;
    if( OffSetPolygon( polygon, offset, isInner, offset_polygon ) )
    {
        ...
        DrawPolygonHatch( mode, offset_polygon, _T( "GRAVEL" ), scale, color );
    }
}
...
Adesk::Boolean SimpleGoafDraw::subWorldDraw( AcGiWorldDraw* mode )
{
    ...
    {
        // 绘制外框
        DrawBoundary( mode, m_pts, m_width * 0.5, false );

        // 绘制采空区填充
        //acutPrintf(_T("\nscale:%.5f"), m_scale);
        const double c = 1.0;
        DrawGoaf( mode, m_pts, m_width * 0.5, c * m_scale, false );
    }
