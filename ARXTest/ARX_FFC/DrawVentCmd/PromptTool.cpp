...
void SetEdgeSEPts( const AcDbObjectIdArray& objIds, AcGePoint3dArray& spts, AcGePoint3dArray& epts )
{
    AcTransaction* pTrans = actrTransactionManager->startTransaction();
    if( pTrans == 0 ) return;
    ...
    for( int i = 0; i < len; i++ )
    {
        ...
        pEdge->setSEPoint( spts[i], epts[i] );
        pEdge->updateDraw();
    }
    actrTransactionManager->endTransaction();
}
...
bool SetSEPt( const AcDbObjectId& objId, const AcGePoint3d& spt, const AcGePoint3d& ept )
{
    AcTransaction* pTrans = actrTransactionManager->startTransaction();
    if( pTrans == 0 ) return false;
...
bool GetClosePtAndAngle( const AcDbObjectId& objId, AcGePoint3d& pt, double& angle )
{
    ...
    if( isArcTunnel )
    {
        CacArcClosePt( startPt, endPt, thirdPt, pt, angle );
    }
    else
    {
