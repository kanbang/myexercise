 ...
 */
#include <map>
struct Reactor_DocumentReactor
{
    Reactor_DocumentReactor() : pTooltipMonitor( 0 ), pEraseDbReactor( 0 ) {}
 ...
 */
typedef std::map<long, Reactor_DocumentReactor> DocumentReactorMap;

LinkedGE_EditorReactor* pEdgeReactor = 0;
DocumentReactorMap* pDocumentReactorMap_Reactor = 0;
...
{
    if( pDocumentReactorMap_Reactor == 0 )
    {
        pDocumentReactorMap_Reactor = new DocumentReactorMap();
    }
}

void ReactorHelper::RemoveDocumentReactorMap()
{
    delete pDocumentReactorMap_Reactor;
    pDocumentReactorMap_Reactor = 0;
}

void ReactorHelper::AddDocumentReactor( AcApDocument* pDoc )
{
    if( pDoc == 0 ) return;
    if( pDocumentReactorMap_Reactor == 0 ) return;
    long id = ( long )pDoc;
    DocumentReactorMap::iterator itr = pDocumentReactorMap_Reactor->find( id );
    if( itr != pDocumentReactorMap_Reactor->end() ) return;

    Reactor_DocumentReactor dr;
    ...
    pDocumentReactorMap_Reactor->insert( DocumentReactorMap::value_type( id, dr ) );
}
