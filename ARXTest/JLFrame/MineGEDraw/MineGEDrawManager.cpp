...
void MineGEDrawManager::regGEDraw( const CString& geType, MineGEDraw* pDraw )
{
    // 初始化默认参数
    pDraw->setAllExtraParamsToDefault();

    MineGEDrawInfo drawInfo = {pDraw};

    GEDrawMap::iterator itr = m_drawMap.find( geType );
    if( itr == m_drawMap.end() ) // 尚未注册该类型
    {
        DrawListInfo info;
        info.m_defDraw = pDraw; // 设定第一个注册的可视化效果为当前显示效果
...
void MineGEDrawManager::unRegGEDraw( const CString& geType, const CString& realGEDrawName )
{
    ...
    if( itr->second.m_defDraw == drawInfo.pDraw )
    {
        if( draw_list.empty() )
        {
            itr->second.m_defDraw = 0; // 那么图元可视化效果为空
        }
        else
        {
            // 如果还有其它的可视化效果，那么使用第一个注册的可视化效果
