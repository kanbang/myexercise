...
{
    // 按照角度大小排序（逆时针）
    std::sort( ges.begin(), ges.end(), SortEdgeByAngle() ); // 不能使用list，只能使用随机容器vector或数组
}

// 新的计算方法，考虑向量长度(巷道宽度)
static AcGeVector3d CaclAverageVector( const AcGeVector3d& v1, double w1, const AcGeVector3d& v2, double w2 )
{
    // 从v1->v2的转角
...
// 旧的计算方法，没有考虑向量长度(巷道宽度)
static AcGeVector3d CaclAverageVector_Old( const AcGeVector3d& v1, const AcGeVector3d& v2 )
{
    double angle = v2.angleTo( v1, -AcGeVector3d::kZAxis );
...
static void EdgeJunctionClosureImpl( const AcGePoint3d& junctionPt, EdgeInfo& ges )
{
    ...
    for( EdgeInfoIter itr = ges.begin(); itr != ges.end(); itr++ )
    {
        ...
        {
            //acutPrintf(_T("\n叉乘长度=%.3f"), cv.length());
            //v3 = CaclAverageVector_Old(itr->angle, itr2->angle);
            v3 = CaclAverageVector( itr->angle, 1, itr2->angle, 1 );
        }
        else
        {
            // 平行(夹角=0或PI)
