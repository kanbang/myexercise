...
AC_IMPLEMENT_EXTENSION_MODULE( ObjectRecordPoolDLL )
...
#ifdef NEVER
AFX_EXTENSION_MODULE ObjectRecordPoolExtDLL = { NULL, NULL } ;
#endif
...
BOOL WINAPI DllMain ( HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved )
{
    ...
    if ( dwReason == DLL_PROCESS_ATTACH )
    {
        ...
        ObjectRecordPoolDLL.AttachInstance ( hInstance ) ;
        InitAcUiDLL () ;
    }
    else if ( dwReason == DLL_PROCESS_DETACH )
    {
        ObjectRecordPoolDLL.DetachInstance () ;
    }
    return ( TRUE ) ;
}
