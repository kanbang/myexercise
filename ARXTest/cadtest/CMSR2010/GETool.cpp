...
// 巷道上是否有构筑物(风门、密闭、风窗)
bool HasGate( const AcDbObjectId& objId )
{
    AcDbObjectIdArray objIds;
    DrawHelper::GetAllTagGEById( objId, objIds );

    // 判断是否有风门(包括永久/临时风门、双向风门、平衡风门)
    // 判断是否有调节风窗
    // 判断是否有风墙
    bool ret = false;
    int len = objIds.length();
    for( int i = 0; i < len; i++ )
    {
        if( ArxUtilHelper::IsEqualType( _T( "Gate" ), objIds[i] ) )
        {
            ret = true;
            break;
        }
        if( ArxUtilHelper::IsEqualType( _T( "Casement" ), objIds[i] ) )
        {
            ret = true;
            break;
        }
        if( ArxUtilHelper::IsEqualType( _T( "Wall" ), objIds[i] ) )
        {
            ret = true;
            break;
        }
