...
AC_IMPLEMENT_EXTENSION_MODULE( VentNetworkAlgoDLL )
...
#ifdef NEVER
AFX_EXTENSION_MODULE VentNetworkAlgoExtDLL = { NULL, NULL } ;
#endif
...
BOOL WINAPI DllMain ( HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved )
{
    ...
    if ( dwReason == DLL_PROCESS_ATTACH )
    {
        ...
        VentNetworkAlgoDLL.AttachInstance ( hInstance ) ;
        InitAcUiDLL () ;
    }
    else if ( dwReason == DLL_PROCESS_DETACH )
    {
        VentNetworkAlgoDLL.DetachInstance () ;
    }
    return ( TRUE ) ;
}
