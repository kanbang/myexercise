...
BOOL CIniFile::SetPath( const CString& strPath )
{
    m_strPath = strPath;
...
BOOL CIniFile::SectionExist( const CString& strSection )
{
...
CString CIniFile::GetKeyValue( const CString& strSection, const CString& strKey )
{
    TCHAR chKey[MAX_KEY];
    DWORD dwRetValue;
...
void CIniFile::SetKeyValue( const CString& strSection,
...
void CIniFile::DeleteKey( const CString& strSection, const CString& strKey )
{
    WritePrivateProfileString(
        ( LPCTSTR )strSection,
        ( LPCTSTR )strKey,
...
int CIniFile::GetAllKeysAndValues( const CString& strSection,
                                   ...
                                   CStringArray& strArrKeyValue )
{
    ...
    for( j = 0; j < i; j++ )
    {
        ...
        if( chAllKeysAndValues[j] == NULL )
        {
            ...
            strArrKey.Add( strTempKey.Left( strTempKey.Find( _T( '=' ) ) ) );
            strArrKeyValue.Add( strTempKey.Mid( strTempKey.Find( _T( '=' ) ) + 1 ) );
            ZeroMemory( chTempkeyAndValue, MAX_KEY );
            iPos = 0;
        }
    }
