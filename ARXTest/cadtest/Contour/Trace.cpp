    ...
    ContourArray& cna )
{
    ...
    for( int i = 0; i < ( int )zValues.size(); i++ )
    {
        ...
        else
        {
            ...
            {
                DT_Contour z_contour = {i, z_cnpos[j], dirs[j]};
                cna.push_back( z_contour );
            }
            // 复制计算得到的等值线数据
            std::copy( z_cnpts.begin(), z_cnpts.end(), std::back_inserter( cnpts ) );
...
{
    Contour cnt( pts, ea, ta );
    cnt.searchBoundary( bpts, bpos );
}

int SearchMaxAreaBoundary( const PointArray& bpts, const IntArray& bpos )
{
    double maxArea = -1;
    int pos = -1;
