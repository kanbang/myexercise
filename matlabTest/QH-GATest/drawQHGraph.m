...
if toDraw
    % 绘制节点文字标注
    drawVertexAnnotation(EdgeRectPos, V, E);
end
...
%             disp(sprintf('第%d条独立通路的总高度%.3f\n', i, H_all(i)));
    ...
    function outH = caclNodeHeightOfEachPath(V, E, H, path)
        ...
        for e = path
            ...
            v = E(e, 3);
            H(v) = H(u) + E(e, 5);
            disp(sprintf('计算分支%d的压能，始点：%d--%.3f, 末点:%d--%.3f\n', E(e,1), V(u), H(u), V(v), H(v)));
        end
        outH = H;
    end
    ...
    function drawVertexText(pt, v)
        ...
        text(pt(1), pt(2), sprintf('v%d', v), 'HorizontalAlignment','center');
    end
