function [sg, tentacle] = dfs_f(E, A, layer, regSource, networkList, u, v)
% 深度优先搜索(正向搜索-查找出边)
...
tentacle = removeErrorTentacleEdges1(E, A, sg, tentacle, u);
    ...
    function [outSG, outTentacle, outTempTentacle, outColor, outGoBack] = dfs1(E, A, layer, regSource, networkList, u, v, sg, tentacle, tempTentacle, color, goBack)
        % 深度优先搜索(正向搜索-查找出边)
        if u == v
            % 表示已到达汇点
            outSG = sg;
        ...
        for e=A{u,1}
            ...
            sg = [sg e]; % 添加到网络的分支集合中
            [outSG, outTentacle, outTempTentacle, outColor, outGoBack] = dfs1(E, A, layer, regSource, networkList, E(e,3), v, sg, tentacle, tempTentacle, color, goBack);
            sg = outSG;
