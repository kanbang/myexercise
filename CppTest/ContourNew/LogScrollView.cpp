 ...
 *
 * 修改了两处1，影响模式
 * 2 OnScrollBy()中ScrollWindows()参数的设置
 * 因为对于CS_OWNDC风格需要使用逻辑单位
 *
 ******************************************/
#include "stdafx.h"
#include <afxpriv.h>
...
//		pDC->SetViewportExt(1440/m_iRatio, 1440/m_iRatio);

        CPoint ptVpOrg( 0, 0 );     // assume no shift for printing

// Next 1 line rem By Robter X.
...
void CLogScrollView::SetLogScrollSizes( SIZE sizeTotal, int iRatio )
{
    m_iRatio = iRatio;
    ...
    //BLOCK: convert logical coordinate space to device coordinates
    {
        ...
        dc.SetViewportExt( 1440 / m_iRatio, 1440 / m_iRatio );
...
void CLogScrollView::UpdateBars()
{
    ...
    // Next 4 lines was added by Robter X.
    // 设置滚动页面大小
    GetScrollInfo( SB_HORZ, &info, SIF_PAGE );
    m_pageDev.cx = info.nPage;
    GetScrollInfo( SB_VERT, &info, SIF_PAGE );
    m_pageDev.cy = info.nPage;
...
BOOL CLogScrollView::OnScrollBy( CSize sizeScroll, BOOL bDoScroll )
{
    ...
    if ( bDoScroll )
    {
        ...
        // Add by Robter X.
        if( CS_OWNDC == ( GetClassLong( this->m_hWnd, GCL_STYLE ) & CS_OWNDC ) )
            ScrollWindow( -( x - xOrig )*m_iRatio, -( y - yOrig )*m_iRatio );
        else
