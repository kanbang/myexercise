...
void CDemoDlg::AddColumns( void )
{
    ...
    m_lstDemo.SetColumnSorting( 3, CListCtrlEx::Auto, CListCtrlEx::String );
...
void CDemoDlg::FillListBox( void )
{
    ...
    for( int i = 0; i < 20; i++ )
    {
        ...
        CString value;
        value.Format( "%ld", RGB2LONG( 255, 255, 255 ) );
        m_lstDemo.SetItemText( i, 3, value );
...
BOOL CDemoDlg::InitEditor( CWnd** pWnd, int nRow, int nColumn, CString& strSubItemText, DWORD_PTR dwItemData, void* pThis, BOOL bUpdate )
{
    ...
    case 3:
    {
        CColorDialog* pDlg = dynamic_cast<CColorDialog*>( *pWnd );
        pDlg->SetCurrentColor( ( COLORREF )atol( strSubItemText ) );
        pDlg->DoModal();
    }
    break;
    }
...
BOOL CDemoDlg::EndEditor( CWnd** pWnd, int nRow, int nColumn, CString& strSubItemText, DWORD_PTR dwItemData, void* pThis, BOOL bUpdate )
{
    ...
    case 3:
    {
        ...
        COLORREF rgb = pDlg->GetColor();

        CString value;
        value.Format( "%ld", ( unsigned long )rgb );
        strSubItemText = value;

        pList->SetCellColors( nRow, nColumn, rgb, -1 );
    }
    break;
    }
    return TRUE;
